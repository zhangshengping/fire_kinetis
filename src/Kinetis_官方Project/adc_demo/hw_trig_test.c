...
// **********************************************************
//
// The following code demonstrates the hardware trigger capability of the two ADCs. 
// 下面的代码展示了两个ADC 硬件转换性能
//
...
// 对于两个硬件转换通道的每个ADC，有两个状态控制寄存器SC1 和 两个结果寄存器
//
...
// [为什么每个ADC都需要两个相同的寄存器设置？这样当一个寄存器在用着，另外一个也可以用。]
//
...
* adc0_isr(void)
*
* use to signal ADC0 end of conversion  ADC0转换中断
* In:  n/a
* Out: n/a
******************************************************************************/
void adc0_isr(void)
