                      ...
                      LIB_ERR     *perr)
{
    ...
    size_req  = Mem_PoolSegCalcTotSize((void     *)pmem_addr,
                                       ...
                                       (CPU_SIZE_T)align_actual);
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
    if (size_req < 1u) {                                            /* If req'd size ovf, ...                           */
        CPU_CRITICAL_EXIT();
...
/*$PAGE*/
    ...
    if (pmem_base_addr == (void *)0) {                              /* If no base addr, cfg mem pool from heap.         */
        ...
        size_tot_ptrs   =  Mem_PoolSegCalcTotSize((void     *)pmem_addr_ptrs,
                                                  ...
                                                  (CPU_SIZE_T)sizeof(void *));
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
        if (size_tot_ptrs < 1u) {                                   /* If heap ovf, ...                                 */
            CPU_CRITICAL_EXIT();
        ...
        size_tot_pool   =  Mem_PoolSegCalcTotSize((void     *)pmem_addr_pool,
                                                  ...
                                                  (CPU_SIZE_T)blk_align_actual);
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
        if (size_tot_pool < 1u) {                                   /* If heap ovf, ...                                 */
            CPU_CRITICAL_EXIT();
           *perr = LIB_MEM_ERR_HEAP_OVF;                            /* ... rtn err but add'l heap size NOT avail.       */
            return;
        }
...
/*$PAGE*/
        ...
        size_tot_pool  =  Mem_PoolSegCalcTotSize((void     *)pmem_addr_pool,
                                                 ...
                                                 (CPU_SIZE_T)blk_align_actual);
#if (LIB_MEM_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
        if (size_tot_pool < 1u) {                                   /* If seg  ovf, ...                                 */
            CPU_CRITICAL_EXIT();
                                            ...
                                            CPU_SIZE_T   blk_align)
{
    ...
    if (blk_nbr > 1u) {
        ...
        blk_size_aligned_nbr = blk_size_aligned * (blk_nbr - 1u);
