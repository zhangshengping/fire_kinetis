                       ...
                       CPU_SIZE_T   len_max)
{
    CPU_CHAR    *p1_str_cmp;
    CPU_CHAR    *p2_str_cmp;
    CPU_CHAR    *p1_str_cmp_next;
    CPU_CHAR    *p2_str_cmp_next;
    CPU_INT16S   cmp_val;
    CPU_SIZE_T   cmp_len;


    if (len_max == 0u) {                                        /* If cmp len = 0,        rtn 0       (see Note #3d1A). */
        return ((CPU_INT16S)0);
    }
    ...
    while ((*p1_str_cmp      == *p2_str_cmp)      &&            /* Cmp strs until non-matching chars (see Note #3c) ... */
           ...
           ( p1_str_cmp_next != (CPU_CHAR *)  0 ) &&            /* ... or NULL ptr(s) found          (see Note #3a2);   */
           ( p2_str_cmp_next != (CPU_CHAR *)  0 ) &&
    ...
    if (cmp_len == len_max) {                                   /* If strs     identical for max len nbr of chars, ...  */
        return ((CPU_INT16S)0);                                 /* ... rtn 0                 (see Note #3d2A).          */
    }
    ...
    if (*p1_str_cmp != *p2_str_cmp) {                           /* If strs NOT identical, ...                           */
        ...
        if (p1_str_cmp_next == (CPU_CHAR *)0) {
            if (p2_str_cmp_next == (CPU_CHAR *)0) {             /* If BOTH next str ptrs NULL, ...                      */
                       ...
                       CPU_CHAR     srch_char)
{
    ...
    if (pstr == (CPU_CHAR *)0) {                                /* Rtn NULL if srch str ptr NULL (see Note #3a1).       */
        return ((CPU_CHAR *)0);
    }

    if (len_max == (CPU_SIZE_T)0u) {                            /* Rtn NULL if srch len = 0      (see Note #3e1).       */
        return ((CPU_CHAR *)0);
    }
...
*                               pstr     = "456"
*
*                   (b) If the number of digits to format ('nbr_dig') is zero; then NO formatting is 
*                       performed except possible NULL-termination of the string (see Note #4).
*
*                           Example :
*
*                               nbr      = 23456
...
*
*                   (a) Leading character MUST be a printable ASCII character.
*
*                   (b) The number of leading characters is such that the total number of integer digits 
*                       plus the number of leading characters is equal to the requested number of integer 
*                       digits to format ('nbr_dig').
*
*                           Example :
*
*                               nbr       = 23456
...
*                               pstr     = "456"
*
*                   (b) If the number to format ('nbr') is negative but the most-significant digits of 
*                       the formatted number are truncated (see Note #2a); then the negative sign still 
...
*                               pstr     = "-456"
*
*                   (c) If the number of digits to format ('nbr_dig') is zero; then NO formatting is 
*                       performed except possible NULL-termination of the string (see Note #4).
*
*                           Example :
*
...
*                   (a) Leading character MUST be a printable ASCII character.
*
*                   (b) (1) The number of leading characters is such that the total number of integer digits 
*                           plus the number of leading characters plus possible negative sign character is 
...
*                                   pstr      = " -23456"
*
*                       (2) If the calculated number of leading characters is zero but the number to format 
*                           ('nbr') is negative; then the negative sign character prefixes the requested 
*                           number of integer digits to format.
*
*                               Example :
*
*                                   nbr       = -23456
...
*                                   pstr      = "-23456"
*
*                       (3) (A) If the number to format ('nbr') is negative AND the leading character 
*                               ('lead_char') is     a valid digit for the number's base ('nbr_base'); 
*                               then the negative sign character prefixes all leading characters prior 
...
*                               ('lead_char') is NOT a valid digit for the number's base ('nbr_base'); 
*                               then the negative sign character immediately prefixes the most 
*                               significant digit of the formatted number.
*
*                                   Examples :
*
*                                       nbr        = -23456
...
*                                       pstr       = "fff-ABCD"                 See Note #3b3C
*
*                           (C) An alphabetic leading character is a valid digit for the number's base 
*                               if it is within the numerical range of the number base AND its case also 
*                               matches that of the lower-case option ('lower_case') :
*
*                               (1) Lower-case alphabetic leading characters within the numerical range 
*                                   of the number base are valid when lower-case option is enabled  (i.e. 
*                                   'lower_case' is 'DEF_YES').
*                               (2) Upper-case alphabetic leading characters within the numerical range 
*                                   of the number base are valid when lower-case option is disabled (i.e. 
*                                   'lower_case' is 'DEF_NO').
*
*                   (c) (1) If the value of the number to format is     zero  ...
*                       (2) ... & the number of digits to format is non-zero, ...
*                       (3) ... but NO leading character available;           ...
...
*               pstr            Pointer to character array to return formatted number string (see Note #5).
*
* Return(s)   : Pointer to formatted string, if NO error(s) [see Note #6].
*
*               Pointer to NULL,             otherwise.
*
* Caller(s)   : Application.
...
*                               pstr    = "456.78"
*$PAGE*
*                   (b) (1) If the number to format ('nbr') is negative but the most-significant digits of 
*                           the formatted number are truncated (see Note #2a); then the negative sign still 
*                           prefixes the truncated formatted number.
*
*                               Example :
...
*                                   pstr    = "-456.78"
*
*                       (2) If the number to format ('nbr') is negative & the number of digits to format 
*                           ('nbr_dig') is zero but the number of decimal point digits to format ('nbr_dp') 
*                           is non-zero; then the negative sign immediately prefixes the decimal point -- 
...
*                       See also Note #1.
*
*                   (d) If the total number of digits to format ('nbr_dig + nbr_dp') is zero; then NO 
...
*
*                       (3) (A) If the number to format ('nbr') is negative AND the leading character 
*                               ('lead_char') is     a base-10 digit; then the negative sign character 
...
*
*                           (B) If the number to format ('nbr') is negative AND the leading character 
*                               ('lead_char') is NOT a base-10 digit; then the negative sign character 
...
*
*                   (b) Lead character is NOT a printable character (see Note #3a).
*                       (1) No string format performed; NULL pointer returned.
*
                          ...
                          CPU_CHAR     *pstr)
{
    ...
    if ((nbr_dig > 0u) || (nbr_dp > 0u)) {                      /* If at least one dig or dp req'd, ...                 */
                                                                /* ... prepare nbr fmt :                                */
        ...
        if (nbr_neg == DEF_YES) {                               /* If nbr neg, ...                                      */
            lead_char_dig = ASCII_IsDig(lead_char);             /* ... chk if lead char a base-10 dig (see Note #3b3).  */
        } else {
            lead_char_dig = DEF_NO;
        }

        nbr_log     = (CPU_FP32  )nbr_fmt;
        nbr_dig_th  = (CPU_INT16U)nbr_dig + LIB_STR_CFG_FP_MAX_NBR_DIG_SIG;
        nbr_dig_max = (CPU_INT16U)0u;
        while ((nbr_log     >= 1.0f) &&                         /* While base 10 digs avail               ...           */
               (nbr_dig_max <  nbr_dig_th)) {                   /* ... but NOT > nbr req'd &/or sig digs, ...           */
            nbr_log /= 10.0f;
            nbr_dig_max++;                                      /* ... calc max nbr digs.                               */
        }
                                                                /* Calc init'l nbr of sig digs NOT fmt'd.               */
        nbr_dig_sig = (nbr_dig_max > (CPU_INT16U)nbr_dig)
                    ? (nbr_dig_max - (CPU_INT16U)nbr_dig) : 0u;
    }


/*$PAGE*/
    if (nbr_dig > 0u) {                                         /* Fmt str for desired nbr digs :                       */
            ...
            if (nbr_dig_sig < LIB_STR_CFG_FP_MAX_NBR_DIG_SIG) { /* If nbr sig digs < max, fmt str digs;           ...   */
                ...
                if ((nbr_shiftd > 0u) ||                        /* If shifted nbr > 0         (see Note #3c1)  ...      */
                   ...
                   *pstr_fmt++ = (CPU_CHAR  )(dig_val    + '0');
                    ...
                    if (nbr_shiftd > 0u) {                      /* If shifted nbr > 0, ...                              */
                        nbr_dig_sig++;                          /* ... inc nbr sig digs.                                */
                    }

                } else if ((nbr_neg       == DEF_YES) &&        /* ... else if nbr neg                         ...      */
                           (lead_char_dig == DEF_YES) &&        /* ... & lead char a base-10 dig               ...      */
                           (nbr_neg_fmtd  == DEF_NO )) {        /* ... but neg sign NOT yet fmt'd,             ...      */
    ...
    if (nbr_dp > 0u) {                                          /* Fmt str for desired nbr dp :                         */
        if ((nbr_neg      == DEF_YES) &&                        /* If  nbr neg                     ...                  */
            (nbr_neg_fmtd == DEF_NO )) {                        /* ... but neg sign NOT yet fmt'd, ...                  */
            *pstr_fmt++    = '-';                               /* ... prepend neg sign (see Notes #2b & #3b).          */
        }
       ...
       *pstr_fmt++ = '.';                                       /* Append dp prior to dp conversion.                    */
        dp_exp = 10.0f;
        for (i = 0u; i < nbr_dp; i++) {
            if (nbr_dig_sig <  LIB_STR_CFG_FP_MAX_NBR_DIG_SIG) {/* If nbr sig digs < max, fmt str dps;    ...           */
               ...
               *pstr_fmt++  = (CPU_CHAR  )(dig_val    + '0');
                ...
                if ((nbr_shiftd  > 0u) ||                       /* If shifted nbr > 0                  ...              */
                    (nbr_dig_sig > 0u)) {                       /* ... OR  > 0 sig digs already fmt'd, ...              */
                     nbr_dig_sig++;                             /* ... inc nbr sig digs.                                */
                }
...
*                                   pstr     = "456"
*
*                       (2) If the number to format ('nbr') is negative but the most-significant digits 
*                           of the formatted number are truncated (see Note #2a); then the negative sign 
*                           still prefixes the truncated formatted number.
*
*                               Example :
*
*                                   nbr      = -23456
...
*                                   pstr     = "-456"
*$PAGE*
*                       (3) If the number of digits to format ('nbr_dig') is zero; then NO formatting 
...
*                                   pstr      = " -23456"
*
*                       (2) If the calculated number of leading characters is zero but the number to 
*                           format ('nbr') is negative; then the negative sign character prefixes the 
*                           requested number of integer digits to format.
*
*                               Example :
*
*                                   nbr       = -23456
...
*                       (1) No string format performed; NULL pointer returned.
*
*                   (c) Lead character is NOT a printable character (see Note #3a).
*                       (1) No string format performed; NULL pointer returned.
*
                                     ...
                                     CPU_CHAR     *pstr)
{
    ...
    if (nbr_dig > 0u) {                                         /* If at least one dig req'd, ...                       */
        nbr_fmt     = nbr;                                      /* ... prepare nbr fmt :                                */
        nbr_log     = nbr;
        nbr_dig_max = 1u;
        while ((nbr_log     >= nbr_base) &&                     /* While nbr base digs avail     ...                    */
               (nbr_dig_max <  nbr_dig )) {                     /* ... but NOT > nbr req'd digs, ...                    */
            nbr_log /= nbr_base;
            nbr_dig_max++;                                      /* ... calc max nbr digs.                               */
        }

        nbr_dig_min   =  DEF_MIN(nbr_dig_max, nbr_dig);         /* Calc  nbr min   digs, ...                            */
        nbr_dig_fmtd  = (lead_char != (CPU_CHAR)'\0')
                      ?  nbr_dig      : nbr_dig_min;            /* ...   nbr fmt'd digs, ...                            */
        nbr_lead_char =  nbr_dig_fmtd - nbr_dig_min;            /* ... & nbr lead chars.                                */

        pstr_fmt     +=  nbr_dig_fmtd;                          /* Start fmt @ least-sig dig.                           */
        nbr_neg_fmtd  =  DEF_NO;

        if (nbr_neg == DEF_YES) {                               /* If nbr neg                                       ... */
            if (nbr_lead_char > 0u) {                           /* ... &      lead chars to fmt,                    ... */
                ...
                if (lower_case != DEF_YES) {
                    ...
                    lead_char_delta_a = (CPU_INT08U)(lead_char - 'a');
                                                                ...
                                                                /* ... chk if lead char a nbr base dig (see Note #3b3); */
            ...
            } else {                                            /* ... but NO lead chars to fmt,                    ... */
                pstr_fmt++;                                     /* ... adv str to reserve neg sign char for later fmt.  */
                lead_char_dig = DEF_NO;
            }
    ...
    if (nbr_dig > 0u) {                                         /* Fmt str for desired nbr digs :                       */
        for (i = 0u; i < nbr_dig_fmtd; i++) {
            if ((nbr_fmt > 0u) ||                               /* If fmt nbr > 0              (see Note #3c1)  ...     */
                (i == 0u)) {                                    /* ... OR on one's  dig to fmt (see Note #3c4), ...     */
                                                                /* ... calc & fmt dig val;                      ...     */
                ...
                nbr_fmt /= nbr_base;                            /* Shift to next more-sig dig.                          */
                     ...
                     (((lead_char_dig == DEF_NO )  &&           /* ... lead char NOT nbr base dig               ...     */
                       (nbr_neg_fmtd  == DEF_NO )) ||           /* ... but neg sign NOT yet fmt'd  OR           ...     */
                      ((lead_char_dig != DEF_NO )  &&           /* ... lead char is  nbr base dig               ...     */
                       (i == (nbr_dig_fmtd - 1u))))) {          /* ... & on most-sig dig to fmt,                ...     */

               *pstr_fmt--   = '-';                             /* ... prepend neg sign (see Notes #1b2 & #3b); ...     */
                nbr_neg_fmtd = DEF_YES;
